        ...
        for antenna_key in positions:
            for i in range(len(positions[antenna_key])):
                ...
                for j in range(len(positions[antenna_key])):
                    ...
                    anitnode_r, antinode_c = delta_r + positions[antenna_key][j][0], delta_c + positions[antenna_key][j][1]

                    if not(anitnode_r < 0 or antinode_c < 0 or anitnode_r >= ROWS or antinode_c >= COLS or (anitnode_r, antinode_c) in unique_antinodes):
